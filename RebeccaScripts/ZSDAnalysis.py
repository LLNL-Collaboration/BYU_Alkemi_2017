...
if __name__ == '__main__':
    ...
    (run, part, cycle, zone) = (6,1,8230,945)
    ...
    while r < 140:
    	    start = time.time()
	    v = 6
	    output = open("BYU_Alkemi_2017/RebeccaScripts/ZSDAnalysis/ZSDRun" + str(r) +"Var" + str(v) + ".csv", "w")
	    output.write("Cycle,SD,FP,FZ,FailedZoneValue,FailedZoneDiff\n")

	    tempData = reader.readAllCyclesForZone(r, 10825)
	    Tcycle = len(tempData)

	    c = 0
	    while c < Tcycle:
		    data = reader.readAllZonesInCycle(r, c)
		    j = data.shape[0]
		    i=0
		    ...
		    FZV = data[zone][v]
		    FSVD = data[zone][v] - Average
		    output.write(str(c) + "," +  str(SD) + "," + str(FP) + "," + str(FZ)
		    	+ "," + str(FZV) + "," + str(FSVD) +"\n")
		    c += 1
	    r += 1
