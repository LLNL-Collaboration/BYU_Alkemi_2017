...
if __name__ == '__main__':
    ...
    (run, part, cycle, zone) = (0, 14, 6899, 10806)
    ...
    times = 0
    while times < 12:
        fileName = "BYU_Alkemi_2017/RebeccaScripts/AvgValues" + str(times) + ".csv"
	OddyFile1 = open(fileName, "w")
	OddyFile1.write("Run,Cycle,Zone,Part,Highest Values,Highest Cycle, Lowest Value," \
	    	"Lowest Cycle,Average Value,Average Slope,Slope > 1,Time Til Crash,Over 100,"\
		"Largest Slope\n")	
	...
	if length == 12000:
        ...
        while z < length:
	    #print z
	    data = reader.readAllCyclesForZone(run, z)
	    j = data.shape[0]
	    i = 0
	    ...
	    while i < j:
	    	#OddyFile1.write(str(i) + "," + str(data[i][6])  + "\n")
		if float(data[i][6]) > x:
			x = data[i][6]
			x1 = i
		if float(data[i][6]) < y:
			y = data[i][6]
			y1 = i
		if float(data[i][6]) >=100:
			LValue += 1
		average += float(data[i][6])
		lines+=1
		ts = (float(data[i][6]) + temp) / 2.0
		slope += ts
		if float(ts) > 1:
			BSlope += 1
		if float(ts) > float(Lslope):
			Lslope = ts
		temp = float(data[i][6])
		OddyFile1.write(str(run) + "," + str(cycle) + "," + str(z) + "," + str(part) + "," + 
			str(x) + "," + str(x1) + "," + str(y) + "," + str(y1) + "," + str(average/lines) + "," +
			str(slope/lines) + "," + str(BSlope) + "," + str(LValue) + "," + str(Lslope)) 
	        i = i+1
	    z += 1
	times +=1
        currTime = time.time()
        print timeStamp - currTime
